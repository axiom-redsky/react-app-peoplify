# 디자인 시스템 종합 가이드 (react-app-scaffold)
...
![디자인 시스템 구조](./design-system-architecture.svg)

> 위 SVG가 안 보이면 [design-system-architecture.svg](./design-system-architecture.svg) 를 직접 연다.
...
| `semantic/light.json`·`dark.json` | **의미 기반** 토큰(primitive 참조) | `primary = {color.brand.500}`<br>`status.active = {color.emerald.500}` |
