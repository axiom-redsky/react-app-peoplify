...
router.get('/', async (req, res, next) => {
  try {
    ...
    const techStack =
      ids.length > 0 ? await db('project_tech_stack').whereIn('project_id', ids) : [];
...
router.post('/', async (req, res, next) => {
  try {
    const { name, client, start_date, end_date, status = 'planned', progress_pct = 0, description, tech_stack = [] } = req.body;
    ...
    if (tech_stack.length > 0) {
      await db('project_tech_stack').insert(
        tech_stack.map((tech) => ({ project_id: project.id, tech })),
      );
    }
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const updatedStack = await db('project_tech_stack')
      .where({ project_id: project.id })
      .pluck('tech');
