		...
		<div className="p-5">
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
				...
				<div className="space-y-4">
					<div className="bg-card rounded-xl border p-5">
						...
						<div className="grid grid-cols-2 gap-3 mb-4">
							<div>
								...
								<input
									type="number"
									defaultValue="22"
									className="w-full px-3 py-2 text-sm border rounded-lg outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20"
								/>
							</div>
							<div>
						...
						<div className="mb-4">
							...
							<textarea
								rows={3}
								className="w-full px-3 py-2 text-sm border rounded-lg outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 resize-none"
					...
					<div className="bg-card rounded-xl border p-4">
						...
						<div className="space-y-2">
							{recentReports.map((r) => (
								<div key={r.month} className="flex items-center justify-between py-2 border-b last:border-0">
									<div>
				...
				<div className="bg-card rounded-xl border p-4">
					...
					<div className="flex items-center gap-2 mb-4 p-2.5 bg-orange-50 rounded-lg border border-orange-100">
					...
					<div className="overflow-x-auto">
						<table className="w-full text-sm">
							...
							<tbody>
								{teamReports.map((m) => (
									<tr key={m.name} className={`border-t transition-colors ${m.status === '미제출' ? 'bg-red-50/50' : 'hover:bg-muted/20'}`}>
										...
										<td className="py-2.5 px-3">
											<span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
												m.status === '승인' ? 'bg-emerald-100 text-emerald-700' :
												m.status === '제출' ? 'bg-sky-100 text-sky-700' :
