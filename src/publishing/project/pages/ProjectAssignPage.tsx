			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
								...
								{benchMembers.map((m) => (
									...
									>
										<td className="py-2.5 px-4">
											<div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${m.selected ? 'bg-teal-600 border-teal-600' : 'border-slate-300 dark:border-slate-600'}`}>
										...
										<td className="py-2.5 px-4">
											<div className="flex items-center gap-2">
												...
												<span className={`font-medium ${m.selected ? 'text-teal-700 dark:text-teal-300' : 'text-foreground'}`}>{m.name}</span>
											</div>
										</td>
				...
				<div className="bg-card rounded-xl border p-4 h-fit">
					...
					<div className="mt-4 pt-4 border-t">
						<p className="text-xs text-orange-600 dark:text-orange-400 mb-3">⚠ 배정 확정 시 투입 현황 자동 갱신 · 해당 직원 이메일 알림 발송</p>
						<div className="flex gap-2">
							<Button
								variant="outline"
								className="flex-1"
