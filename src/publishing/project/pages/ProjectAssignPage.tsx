		...
		<div className="p-5">
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
				...
				<div className="lg:col-span-2">
					...
					<div className="bg-card rounded-xl border overflow-hidden">
						...
						<table className="w-full text-sm">
							...
							<tbody>
								{benchMembers.map((m) => (
									<tr key={m.id} className={`border-t transition-colors ${m.selected ? 'bg-teal-50' : 'hover:bg-muted/20'}`}>
										<td className="py-2.5 px-4">
											<div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${m.selected ? 'bg-teal-600 border-teal-600' : 'border-gray-300'}`}>
										...
										<td className="py-2.5 px-4">
											<div className="flex items-center gap-2">
												...
												<span className={`font-medium ${m.selected ? 'text-teal-700' : 'text-foreground'}`}>{m.name}</span>
											</div>
										</td>
				...
				<div className="bg-card rounded-xl border p-4 h-fit">
					...
					<div className="mb-3 p-2.5 bg-teal-50 rounded-lg">
						<p className="text-xs text-teal-600 font-medium mb-1">선택 인력:</p>
						<p className="text-sm text-teal-800 font-semibold">김민준, 정다은</p>
					</div>
					...
					<div className="space-y-3">
						...
						<div>
							...
							<div className="flex gap-2">
								<button className="flex-1 py-2 text-sm border rounded-lg text-center bg-teal-600 text-white font-medium">100%</button>
								<button className="flex-1 py-2 text-sm border rounded-lg text-center hover:bg-muted transition-colors text-muted-foreground">50%</button>
							</div>
						</div>
						<div>
							<label className="block text-sm font-medium text-foreground mb-1">투입 시작일 *</label>
							<input type="date" defaultValue="2026-06-01" className="w-full px-3 py-2 text-sm border rounded-lg outline-none focus:border-teal-500" />
						</div>
						<div>
							<label className="block text-sm font-medium text-foreground mb-1">종료 예정일</label>
							<input type="date" defaultValue="2026-12-31" className="w-full px-3 py-2 text-sm border rounded-lg outline-none focus:border-teal-500" />
						</div>
					</div>

					<div className="mt-4 pt-4 border-t">
						<p className="text-xs text-orange-600 mb-3">⚠ 배정 확정 시 투입 현황 자동 갱신 · 해당 직원 이메일 알림 발송</p>
						<div className="flex gap-2">
							<Button variant="outline" className="flex-1" size="sm">초기화</Button>
							<Button className="flex-1" size="sm">배정 확정</Button>
						</div>
					</div>
				</div>
			</div>
