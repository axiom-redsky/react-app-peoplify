		...
		<div className="p-5">
			...
			<div className="flex flex-wrap gap-2 mb-4">
				<div className="relative flex-1 min-w-48">
					...
					<input className="w-full pl-9 pr-3 py-2 text-sm border rounded-lg outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20" placeholder="프로젝트명·고객사 검색..." />
				</div>
			...
			<div className="grid grid-cols-1 gap-3">
				{projects.map((proj) => (
					<div key={proj.id} className="bg-card rounded-xl border p-4 hover:border-teal-200 hover:shadow-sm transition-all">
						<div className="flex items-start justify-between gap-3">
							<div className="flex-1 min-w-0">
								...
								<div className="flex flex-wrap gap-4 text-sm">
									...
									<span className="text-muted-foreground">👥 투입 {proj.count}명</span>
									<span className="text-muted-foreground">📊 투입률 {proj.rate}</span>
								</div>
								<div className="flex flex-wrap gap-1.5 mt-2">
									{proj.tech.map((t) => (
										<span key={t} className="px-2 py-0.5 text-xs rounded bg-slate-100 text-slate-600">
											{t}
										</span>
									))}
								</div>
